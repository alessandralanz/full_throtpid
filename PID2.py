...
max_speed = desired_speed = 0.2
sector_width_degree = 90
half_sw = sector_width_degree / 2
center_left = 90
center_right = 270

leftSide = (int((center_left - half_sw) % 360),
            int((center_left + half_sw) % 360))
rightSide = (int((center_right - half_sw) % 360),
            int((center_right + half_sw) % 360))
...
def update_lidar():
   ...
   # total = len(raw) - 1 #intervals in 270 degrees
    ...
    for deg in range(360):
        ...
        if abs(raw_angle) > 135:
            scan360.append(30) #small padding
            continue
...
def update():
    ...
    # 2) get left/right sector readings
    left_vals  = get_sector_samples(scan360, leftSide)
    right_vals = get_sector_samples(scan360, rightSide)

    # 3) find median distances
    m_left  = getMedian(left_vals)
    m_right = getMedian(right_vals)

    # 4) front median +/- 30 degrees
    FRONT_SWEEP = 30
    front_sector = ((360 - FRONT_SWEEP) % 360, FRONT_SWEEP)
    front_vals   = get_sector_samples(scan360, front_sector)
    m_front      = getMedian(front_vals)

    # 5) if there’s a wall within ~200 mm AHEAD start turning early
    SAFE_FRONT_DISTANCE = 200  #raise to turn sooner
    if m_front < SAFE_FRONT_DISTANCE:
        #steer toward whichever side has more room
        steer =  1.0 if m_right > m_left else -1.0

    else:
        # 6) normal wall following PID
        steer = pidControl(m_left, m_right)

    # 7) clamp and drive
    steer = max(min(steer, 1.0), -1.0)
    rc.drive.set_speed_angle(max_speed, steer)
